...
func AutoMigrateSchema(db *gorm.DB, l *slog.Logger) error {
    ...
    if enable == "" { enable = os.Getenv("GORM_AUTO_MIGRATE") }
    switch enable {
    ...
    if err := db.AutoMigrate(&m.TestCase{}, &m.Step{}); err != nil {
...
func buildDSNFromSplitEnv() string {
    ...
    if port != "" { u.Host = fmt.Sprintf("%s:%s", host, port) }
    if pass != "" {
    ...
    if sslcert != "" { q.Set("sslcert", sslcert) }
    if sslkey != "" { q.Set("sslkey", sslkey) }
    if sslroot != "" { q.Set("sslrootcert", sslroot) }
    u.RawQuery = q.Encode()
    return u.String()
}
