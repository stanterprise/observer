              ...
              >
                ...
                >
                  <div className="flex items-center gap-3">
                    {isOpen ? (
                      <ChevronDown className="h-5 w-5 text-(--stitch-on-surface-muted)" />
                    ) : (
                      <ChevronRight className="h-5 w-5 text-(--stitch-on-surface-muted)" />
                    )}
                ...
                {/* Accordion Body */}
                {isOpen && (
                  <div className="p-4 bg-(--stitch-surface-card) border-t border-(--stitch-outline)">
                    {/* Attempt Info */}
                    <div className="mb-4 p-3 bg-(--stitch-surface-low) rounded-lg">
                      <dl className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        ...
                        {attempt.startTime && (
                          <div>
                            ...
                            <dd className="text-(--stitch-on-surface) mt-1">
                              {new Date(attempt.startTime).toLocaleString()}
                            </dd>
                          </div>
                        )}
                        ...
                        {attempt.duration !== undefined && (
                          <div>
                            ...
                            <dd className="text-(--stitch-on-surface) mt-1 font-semibold">
                    ...
                    {(attempt.errorMessage ||
                      ...
                      <div className="mb-4 p-4 bg-(--status-failure-soft) border border-(--status-failure-border) rounded-lg">
                        <h4 className="text-sm font-semibold text-(--status-failure) mb-2">
                          Error Details
                        </h4>
                        {attempt.errorMessage && (
                          <p className="text-sm text-(--status-failure) mb-2">
                            {attempt.errorMessage}
                          </p>
                        )}
                        {attempt.stackTrace && (
                          <pre className="text-xs text-(--status-failure) bg-(--status-failure-soft) p-2 rounded overflow-x-auto whitespace-pre-wrap">
                            {attempt.stackTrace}
                          </pre>
                        )}
                        {attempt.errors && attempt.errors.length > 0 && (
                          <ul className="text-sm text-(--status-failure) list-disc list-inside space-y-1">
