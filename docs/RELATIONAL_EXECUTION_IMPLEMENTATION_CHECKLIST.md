...
- [ ] Add boot-time validation that PG DSN is reachable.
...
- [ ] Implement atomic step begin/step end mutations using MongoDB `$set` and `$push`.
- [ ] Enforce invariants on attempt index progression in code.
- [ ] Handle duplicate NATS event deliveries idempotently via event key.
